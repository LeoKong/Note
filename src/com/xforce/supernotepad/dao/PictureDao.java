...
public class PictureDao {
	...
	public List<PictureModel> getPicFromNid(String nid) {
		...
		Cursor cursor = sqLiteDatabase.query("tb_pic", new String[] { "p_name",
				"illustration" }, "n_id=?", new String[] { nid }, null, null,
				null);
	...
	public List<PictureModel> getAllPictures() {
		...
		Cursor cursor = null;
		try {
			cursor = sqLiteDatabase.query("tb_pic", new String[] { "p_name",
					"illustration" }, null, null, null, null, "p_id desc");
			while (cursor.moveToNext()) {
				PictureModel pictureModel = new PictureModel();
				pictureModel.setpName(cursor.getString(0));
				pictureModel.setIllustration(cursor.getString(1));
	...
	public boolean checkHasPic() {
		Cursor cursor = null;
		try {
			cursor = sqLiteDatabase.rawQuery("select p_id from tb_pic limit 1",
					null);
